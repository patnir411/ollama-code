...
You can enable telemetry in multiple ways. Configuration is primarily managed via the [`.ollama/settings.json` file](./cli/configuration.md) and environment variables, but CLI flags can override these settings for a specific session.
...
1.  **Workspace settings file (`.ollama/settings.json`):** Values from the `telemetry` object in this project-specific file.

1.  **User settings file (`~/.ollama/settings.json`):** Values from the `telemetry` object in this global user file.
...
The following code can be added to your workspace (`.ollama/settings.json`) or user (`~/.ollama/settings.json`) settings to enable telemetry and send the output to Google Cloud:
...
Use the `npm run telemetry -- --target=local` command to automate the process of setting up a local telemetry pipeline, including configuring the necessary settings in your `.ollama/settings.json` file. The underlying script installs `otelcol-contrib` (the OpenTelemetry Collector) and `jaeger` (The Jaeger UI for viewing traces). To use it:
...
1.  **Inspect logs and metrics**:
    The script redirects the OTEL collector output (which includes logs and metrics) to `~/.ollama/tmp/<projectHash>/otel/collector.log`. The script will provide links to view and a command to tail your telemetry data (traces, metrics, logs) locally.
...
Use the `npm run telemetry -- --target=gcp` command to automate setting up a local OpenTelemetry collector that forwards data to your Google Cloud project, including configuring the necessary settings in your `.ollama/settings.json` file. The underlying script installs `otelcol-contrib`. To use it:
...
1.  **Run the command**:
    ...
    - Automatically enable telemetry and disable sandbox mode in your workspace settings (`.ollama/settings.json`).
...
1.  **Inspect local collector logs**:
    The script redirects the local OTEL collector output to `~/.ollama/tmp/<projectHash>/otel/collector-gcp.log`. The script provides links to view and command to tail your collector logs locally.
